...
fig = make_subplots(rows=2, cols=2, shared_yaxes=True, shared_xaxes=True,
                    subplot_titles=("<b>Old Buildings & Instagram</b>", "<b>Wartime Destruction & Instagram</b>",
                                    "<b>Old Buildings & Overnight Stays</b>", "<b>Wartime Destruction & Overnight Stays</b>"),
                    specs=[[{}, {}], [{}, {}]], horizontal_spacing=0.01, vertical_spacing=0.06)

fig.add_trace(scattertrace(rawData, "old_per_capita",
              "instagram_post_count_cap"), 1, 1)
fig.add_trace(scattertrace(rawData, "destruction",
              "instagram_post_count_cap"), 1, 2)
fig.add_trace(scattertrace(rawData, "old_per_capita",
              "overnights_per_capita_2019"), 2, 1)
fig.add_trace(scattertrace(rawData, "destruction",
              "overnights_per_capita_2019"), 2, 2)

fig.add_trace(olstrace(rawData, "old_per_capita",
              "instagram_post_count_cap"), 1, 1)
fig.add_trace(olstrace(rawData, "destruction",
              "instagram_post_count_cap"), 1, 2)
fig.add_trace(olstrace(rawData, "old_per_capita",
              "overnights_per_capita_2019"), 2, 1)
fig.add_trace(olstrace(rawData, "destruction",
...
fig.update_xaxes(title_text="Old Buildings per Capita",
                 showgrid=True, row=2, col=1)
fig.update_xaxes(title_text="Wartime Destruction (%)",
                 showgrid=True,  row=2, col=2)

# Update yaxis properties
fig.update_yaxes(title_text="Instagram Posts per Capita",
                 showgrid=True, row=1, col=1)
# fig.update_yaxes(title_text="yaxis 2 title", showgrid=True, row=1, col=2)
fig.update_yaxes(title_text="Overnights per Capita",
                 showgrid=True, row=2, col=1)
...
fig = make_subplots(rows=4, cols=1, shared_yaxes=False, shared_xaxes=False,
                    subplot_titles=("<b>Buildings & Instagram</b>", "<b>Destruction & Instagram</b>",
                                    "<b>Buildings & Overnights</b>", "<b>Destruction & Overnights</b>"),
                    specs=[[{}], [{}], [{}], [{}]], horizontal_spacing=0.01, vertical_spacing=0.1)

fig.add_trace(scattertrace(rawData, "old_per_capita",
              "instagram_post_count_cap"), 1, 1)
fig.add_trace(scattertrace(rawData, "destruction",
              "instagram_post_count_cap"), 2, 1)
fig.add_trace(scattertrace(rawData, "old_per_capita",
              "overnights_per_capita_2019"), 3, 1)
fig.add_trace(scattertrace(rawData, "destruction",
              "overnights_per_capita_2019"), 4, 1)
...
fig.update_xaxes(title_text="Old Buildings  p. C.",
                 showgrid=True, row=3, col=1)
fig.update_xaxes(title_text="Destruction (%)", showgrid=True,  row=4, col=1)

# Update yaxis properties
fig.update_yaxes(title_text="Instagram Posts p. C.",
                 showgrid=True, row=1, col=1)
fig.update_yaxes(title_text="Instagram Posts p. C.",
                 showgrid=True, row=2, col=1)
...
def scatterplot(data, xVar, yVar, xtitle, ytitle):
    ...
    err_size_regr = LinearRegression()
    err_size_res = err_size_regr.fit(
        np.array(xdata).reshape(-1, 1), np.array(ydata))
    err_fit = err_size_regr.predict(np.array(xdata).reshape(-1, 1))
    ...
    fig.add_trace(
        go.Scatter(x=xdata, y=err_fit, mode="lines", name="Error fit",
                   marker_color='rgba(122, 34, 15, 0.8)'),
        secondary_y=False)
